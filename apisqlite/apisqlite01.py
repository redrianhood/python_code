...
def lookup_by_type(mykey, searchsting, typestring):
    try:
        # construct API
        api = f"{OMDBURL}apikey={mykey}&s={searchsting}&type={typestring}"

        # get response and check for 'ok'
        resp = requests.get(api)
        # read, decode JSON to Python, and return
        return resp.json()
    except:
        return False
    ...
    while True:
        ...
        if answer in ["1", "2"]:
            ...
            if answer == "1":
                ...
                print("\nSearch by type coming now!\n")  # maybe you can write this code!
                # continue                                  # restart the while loop
                typestring = input("What type of show?\n"
                                   "[movie, series, or episode]\n"
                                   ">>>")
                resp = lookup_by_type(mykey, searchstring, typestring)
            if resp:
                # display the results
